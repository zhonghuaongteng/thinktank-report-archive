...
output: ../pages/06-topic-open-weight-ai-models-may-increase-biological-misuse-risks-asses.jpg
aspect_ratio: "16:9"
...
- 机构：RAND
- 标题：Open-Weight AI Models May Increase Biological Misuse Risks: Assessing Anti-Refusal Tampering, Capability Enhancement, and Publicly Available Uncensored Models
- 原文链接：https://www.rand.org/pubs/research_reports/RRA5112-1.html
- 优先级：P0
- 主题标签：AI治理, 科技创新

## 需要表达的核心内容

- 核心判断：作者据此认为开放权重带来创新和可访问性收益，但安全对齐不能被视为一次性防线。
- 主要论据：RAND测试开放权重大模型被用户修改后对生物安全的影响，区分“移除拒答保护”和“增强生物学能力”两条路径。；研究发现，移除安全训练在技术上可行，可能放大现实的生物误用路径；直接提升模型的生物能力则更困难。；主要证据来自模型实验和威胁路径评估，不能直接推断现实攻击概率。；报告建议将模型访问控制、危险能力测试和生物实验环节的筛查结合，并承认过度限制会压缩正常研究与开源创新。
...
- 最终输出为可嵌入周报的单张图片，文件名应为 `06-topic-open-weight-ai-models-may-increase-biological-misuse-risks-asses.jpg`。
