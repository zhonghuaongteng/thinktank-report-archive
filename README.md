...
Weekly briefs also reserve a front `漫画导读` section. Markdown, HTML, and PDF weekly outputs now render the comic opener before the overview section. The current candidate style is the single-thesis evidence-chain sample under `comic/weekly-tech-watch-v4/`; it uses one analytical claim per page, report signals as evidence cards, a visible policy bottleneck, and Shanghai-facing action questions. `comic/weekly-tech-watch-v3/`, `comic/weekly-tech-watch-v2/`, and the older four-panel sample are retained only as comparison candidates. Automated comic generation should be wired only after the visual style is accepted.
...
- `comic/weekly-tech-watch-v4/`: current single-thesis evidence-chain sample analysis, storyboard, and prompt for weekly comic openers.
- `comic/weekly-tech-watch-v3/`, `comic/weekly-tech-watch-v2/`, `comic/weekly-tech-watch-sample/`: older comparison samples.
