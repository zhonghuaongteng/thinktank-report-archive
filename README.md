...
Each archived Markdown file treats the Chinese summary as a research card rather than a short abstract. `核心观点` is the body of the card and should normally run 4-8 sentences covering: the report's subject and context, the institution's core judgment including its stance (support/opposition/warning/optimism/skepticism and intensity), the main evidence (data, cases, methods), and internal debates or counterpoints when the report acknowledges them. A one-line core summary is treated as too thin: the weekly audit sidecar lists such P0/P1 items under `核心观点待充实` and they should be rewritten from the source before the weekly brief is rendered. `建议` and `中国/上海参考` are optional sections written only when the report genuinely contains them.

Since July 2026, the three Chinese summary fields are force-deduplicated at render time: sentences in `建议` or `中国/上海参考` that repeat `核心观点` are dropped, and a section that adds no report-specific signal (including placeholder phrases such as 未检出/未提供/待补充) is omitted entirely rather than filled with boilerplate. Not every report has policy advice or a China/Shanghai angle, and cards, weekly pages, PDF topic pages and comic prompts all render these sections adaptively - only when genuine content exists. `核心观点` is additionally split into a one-sentence core judgment plus a main-evidence list so each topic card reads as 判断→论据→建议→参考.

Weekly briefs open with a synthesis layer before any per-item material: `本周态势` (situation summary), `本周必读` (3-5 must-read items with one-line judgments, institution-diverse), and `议题观点速览` (per-chapter cross-institution viewpoint scan). The topic pages then render one card per P0/P1 item with 核心判断, 主要论据, 政策建议, 中国/上海参考 and a tracking question.

Weekly briefs also reserve a front `漫画导读` section. Markdown, HTML, and PDF weekly outputs now render the comic opener before the overview section. The current target style is the infographic-comic standard written into `write_weekly_comic_prompts`: one 16:9 information-poster page per P0/P1 topic with a 20-30-character Chinese headline banner, a single metaphor-driven main visual, 3-4 icon modules, and a four-color palette (#F7F3EA/#1F3A5F/#B84C3D/#C89B52). Generated pages retain enough readable Chinese evidence and short conclusions to stand alone; text is reduced only by removing duplication or reorganizing hierarchy. The previous evidence-chain sample under `comic/weekly-tech-watch-v7/` is retained for comparison; it anchors one report, shows the mechanism and transmission chain, then pairs the image with a textual `读图说明` for the core argument, recommendation, and China/Shanghai reference. `comic/weekly-tech-watch-v6/`, `comic/weekly-tech-watch-v5/`, `comic/weekly-tech-watch-v4/`, `comic/weekly-tech-watch-v3/`, `comic/weekly-tech-watch-v2/`, and the older four-panel sample are retained only as comparison candidates. Automated comic generation should be wired only after the visual style is accepted and generated Chinese text is manually checked.

## Outputs

- Local workspace target: `C:\Users\WINDOWS\OneDrive\知识库\智库信息爬虫`.
- `archive/{institution}/{year}/`: one Markdown file per article/report.
- `briefs/weekly/{year}/`: weekly Markdown, magazine-style print HTML, and PDF briefs. The weekly PDF is produced by printing the magazine HTML with a headless Edge/Chrome (`--print-to-pdf`); set `THINKTANK_PDF_BROWSER` to point at a specific browser binary, or `THINKTANK_DISABLE_BROWSER_PDF=1` to force the legacy reportlab renderer, which also serves as the automatic fallback when no browser is available.
...
The current weekly contract is [economic and enterprise research monitoring](docs/weekly_monitoring_contract_2026-09.md). It adds 16 monitored sources, preserves keyword misses for editorial review, uses exactly seven publication dates, retries unarchived detail failures, and records content, synchronization and delivery separately. [Research interests](docs/research_attention_2026-09.md) add discovery leads across all sources without raising scores or narrowing scope. The opening page is an authored editorial guide with notable findings, relevant research questions and compact navigation, replacing four mandatory front pages. Comics and short introductions lead into substantial, fully rendered research selections, cases and charts; selections may continue on later pages. Source access, full-text evidence and editorial quality require actual verification. See the [five-run audit](docs/audits/2026-09-23_weekly_monitoring_review.md).
