...
- 同一优先级内，写入队列和每日简报重点区优先保存/展示命中“科技创新、半导体、先进制造、数字经济、科技人才、国防AI”的候选，再保存/展示纯AI治理候选，以减少小批量写入和简报展开区被治理类材料挤占。
- 当每日抓取或回填命令设置 `write-limit` 时，若候选池中存在足够的创新支撑材料，有限写入批次至少保留一半名额给创新支撑条目；纯治理专门源保留监测，但不应挤占产业、科研、人才、技术扩散和创新金融等支撑科技创新发展的材料。
- `config/search_profiles.yaml` 提供可审计检索画像。`broad_innovation_support` 作为后续展开检索和批量回填默认画像，保留科技创新、半导体、先进制造、数字经济、科技人才、国防AI等创新支撑条目，并排除只命中AI治理或科技治理的纯治理候选；`ai_governance_watch` 用于需要集中观察治理议题的专题运行。每日监测默认保留全量评分结果，可在命令中显式传入 `--search-profile broad_innovation_support`。
