...
- `config/search_profiles.yaml` 提供可审计检索画像。`broad_innovation_support` 作为后续展开检索、每日监测和批量回填的默认画像，保留科技创新、半导体、先进制造、数字经济、科技人才、国防AI等创新支撑条目，并排除只命中AI治理或科技治理的纯治理候选；`ai_governance_watch` 仅用于需要集中观察治理议题的专题运行。
