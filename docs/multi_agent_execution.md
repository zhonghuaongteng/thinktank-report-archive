...
0. 主控先识别当前运行模式。若用户最新指令为“只优化策略”“不再筛选”“不再回填”或同等含义，进入策略优化模式：不得启动来源评估agent，不得运行 `evaluate`、`backfill`、`run_daily` 或 `run_weekly`，只允许修改策略文档、规则说明、测试和只读包装脚本。
...
- 用户一旦明确暂停筛选或回填，主控必须立即关闭仍在运行的来源评估agent，后续不得再等待或使用其候选输出。
- 若暂停指令到达前已经误触发写入，主控必须把本轮未提交新增归档、SQLite 记录、知识库 CSV 记录和周报重建结果恢复到写入前状态。
- 暂停状态下允许保留的成果仅限策略层资产：协议、文档、测试、只读工具和质量闸门。
- 暂停状态下的默认核验入口为 `scripts\run_strategy_review.ps1`；该脚本只检查策略测试、空白错误和归档/状态/知识库计数，不能替代来源评估或回填。
- 策略优化轮次必须有停止边界：限定待处理问题、限定产出文件、限定验证次数。达到 `docs\retrieval_strategy.md` 中的“本轮优化停止机制”后，主控停止继续派发agent或扩大规则，并进入提交、推送和记忆库记录。
- 恢复筛选、周报或回填必须先有用户明确指令；不能由 agent 根据目标上下文自行恢复。
