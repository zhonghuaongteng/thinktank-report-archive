...
| 漫画导读失焦 | 图片看不出议题关键点 | 当前采用逐条 P0/P1 的 Codex 单篇报告证据链漫画，嵌入每个主题页 |
...
- PDF 阅读版以“核心观点与论述”为主卡片，并将建议与中国/上海参考合并为“政策含义与参考”卡片；不再单列追踪问题。
...
- 每条 P0/P1 主题页嵌入一张漫画。
- 每张图只表达一个主判断。
- 结构为“单篇报告 -> 关键信号 -> 核心机制/矛盾 -> 政策含义 -> 核心结论”。
- 机构名称只作为来源条，视觉重心放在报告核心观点和证据链。
...
- 知识库索引新增行字段完整。
- 周报 Markdown/HTML/PDF 同步更新；逐条 P0/P1 漫画必须使用 Codex 生成图片，`check-weekly-comics` 应确认提示词、图片、Markdown、HTML 和 PDF 数量不低于当期 P0/P1 条目数。
