...
| 漫画导读失焦 | 图片看不出议题关键点 | 当前采用逐条 P0/P1 的 Codex 单篇报告观点科普漫画，嵌入每个主题页 |
...
- 本周态势。
- P0/P1 逐主题页。
- 每个主题页嵌入一张 Codex 生成漫画。
- PDF 阅读版以“核心观点与论述”为主卡片，按“报告对象 -> 中心判断 -> 论述线索 -> 影响路径”展开，并将建议与中国/上海参考合并为“政策含义与参考”卡片；不再单列追踪问题。
...
1. `核心观点`：说明作者中心判断、问题定义、论述线索和关键证据；PDF 阅读版应达到“不读原报告也能把握核心内容”的密度。
2. `建议`：抽取对政府、企业、科研机构、国际合作或治理工具的行动主张；若原文没有明确建议，写明“未检出明确政策建议”。
3. `中国/上海参考`：区分直接涉华/涉沪判断与一般比较参照；没有直接信号时不得强行生成涉沪结论。

### 逐主题漫画

当前默认风格：Codex 生成的“单篇报告观点科普漫画”。
位置：`comic/weekly-topic-comics-<日期>/`

原则：

- 每条 P0/P1 主题页嵌入一张漫画。
- 每张图只表达一个主判断。
- 结构为“单篇报告 -> 文章观点 -> 可视化场景 -> 主要影响 -> 核心结论”。
- 漫画不要求完整复刻报告论证链，重点是通过科普化、可视化视角呈现文章观点。
- 机构名称只作为来源条，视觉重心放在报告核心观点、可视化场景和主要影响。
- 若报告正文、摘要或图注中出现图表、数据曲线、地图、技术架构图、供应链图、照片或其他证据配图线索，可转译为漫画中的报告页、屏幕、白板、证据卡或背景装置；只有自动化提供真实参考图时才纳入原图参考。
- 最后一格或最后一页不强行落到中国或上海；只有报告本身、摘要或主题标签存在明确涉华、涉沪或可操作参照时，才纳入中国/上海内容。
- v2-v7 和旧四格样张只作对照，不作为默认生成风格。
...
- 知识库索引新增行字段完整。
- 周报 Markdown/HTML/PDF 同步更新；逐条 P0/P1 漫画必须使用 Codex 生成图片，`check-weekly-comics` 应确认提示词、图片、Markdown、HTML 和 PDF 数量不低于当期 P0/P1 条目数；PDF 核心观点页不得只复述标题或一句短摘要，漫画提示词必须包含科普化、可视化和证据配图转译要求。
